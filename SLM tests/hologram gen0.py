...
ImageThree = numpy.empty([width.value*height.value*bytpesPerPixel], numpy.uint8, 'C');
InputImg = numpy.empty([width.value*height.value*bytpesPerPixel], numpy.uint8, 'C');
...
try:
    ...
    image_lib.Generate_FresnelLens(ImageTwo.ctypes.data_as(POINTER(c_ubyte)), WFC.ctypes.data_as(POINTER(c_ubyte)), 
                      width.value, height.value, depth.value,
                    center_x, center_y, 300, 200, False, False, RGB);
    
    image_lib.Initialize_HologramGenerator(width.value, height.value, depth.value, 3, 1); 
    
    # image_lib.int GerchbergSaxton(ImageThree.ctypes.data_as(POINTER(c_ubyte)), InputImg.ctypes.data_as(POINTER(c_ubyte)), 
    #                               WFC.ctypes.data_as(POINTER(c_ubyte)), width.value, height.value, depth.value, 3, 1);
    ...
    slm_lib.Delete_SDK();
out = numpy.reshape(ImageTwo, (1920,1200,4), 'C')
plt.imshow(out)
plt.show()